# Repository and Product Relationships

This map preserves every source repository in the portfolio while explaining shared product history.

| Source repository identifier | Display name | Relationship | Public material |
|---|---|---|---|
| `yo20ywork-max/danieldowork-web` | DanielDoWork — AI Workspace | Main product application | [Open](https://github.com/yo20ywork-max/danieldowork-showcase) |
| `yo20ywork-max/danieldowork-ai` | DanielDoWork — AI Infrastructure | AI services integrated with the workspace application | [Open](https://github.com/yo20ywork-max/danieldowork-ai-showcase) |
| `millenniume/crossgen` | CrossGen — Social & Workflow Platform | Company-maintained CrossGen product line | [Open](https://github.com/yo20ywork-max/crossgen-showcase) |
| `yo20ywork-max/crossgen-lab` | CrossGen — Architecture & CI | Related engineering branch of the CrossGen codebase | [Open](https://github.com/yo20ywork-max/crossgen-lab-showcase) |
| `yo20ywork-max/community-os` | Community OS — Collaboration Prototype | Static prototype plus a separate application starter | [Open](https://github.com/yo20ywork-max/community-os-showcase) |
| `yo20ywork-max/tqcpb-web` | Tianqing — Professional Services Website | Client website | [Open](https://github.com/yo20ywork-max/tqcpb-showcase) |
| `millenniume/millennium-web` | Millennium — CMS & Customer Service | Company-maintained CMS repository | [Open](https://github.com/yo20ywork-max/millennium-web-showcase) |
| `yo20ywork-max/millennium-web-legacy` | Millennium — CMS Development Line | Related earlier development repository | [Open](https://github.com/yo20ywork-max/millennium-cms-showcase) |
| `yo20ywork-max/chatgpt-codex-bridge` | ChatGPT–Codex Bridge | Standalone public integration tool | [Open](https://github.com/yo20ywork-max/chatgpt-codex-bridge) |
| `yo20ywork-max/dev-context` | Development Context & Handoff | Shared engineering context | [Open](https://github.com/yo20ywork-max/dev-context-showcase) |
| `2ykrrmyscg-del/FocusOYL` | FocusOYL — Local AI Translator & Agent | Windows translator and a distinct cross-platform agent track | [Open](https://github.com/2ykrrmyscg-del/FocusOYL) |
| `yo20ywork-max/FocusOYL-MathMorph` | FocusOYL-MathMorph — Model Editing Research | Separate research project on GGUF weight editing and evaluation | [Open](https://github.com/yo20ywork-max/FocusOYL-MathMorph) |
| `yo20ywork-max/focus-oyl-reminders` | Focus Oyl — Local Reminders & Care Companion | Separate reminder application imported from the original ultramax folder | [Open](https://github.com/yo20ywork-max/focus-oyl-reminders) |
| `yo20ywork-max/privacy-display` | Privacy Display — Android Privacy Overlay | One app with an initial prototype and a later expanded implementation | [Open](https://github.com/yo20ywork-max/privacy-display) |
| `yo20ywork-max/gameboost` | GameBoost — Network Route Evaluation | One system containing backend, desktop client, and Linux node agent | [Open](https://github.com/yo20ywork-max/gameboost) |

## Product relationships

- DanielDoWork's web application and AI infrastructure run as distinct components. Updating the infrastructure repository alone does not update the web application's deployed frontend.
- The two CrossGen repositories share history and contain different engineering work. Architecture and CI work is presented as a contribution to CrossGen.
- The two Millennium CMS repositories belong to the same product lineage. Their separate case studies show implementation and maintenance context.
- FocusOYL's Windows translator and MiniCPM-based agent use different model/runtime paths. Their validation results are not interchangeable.

- FocusOYL-MathMorph is a model-editing research project. The FocusOYL translator/agent and Focus Oyl Reminders are distinct applications; their evidence is not interchangeable.
- Privacy Display's two source directories are development iterations of one project.
- GameBoost's three runtime components belong to one networking system.

Repository names were standardized for navigation. Application folder names and runtime service names remain independently configured. Previous GitHub repository URLs redirect to the renamed repositories.

## Previous repository names

| Previous identifier | Current identifier |
|---|---|
| `yo20ywork-max/overpower` | `yo20ywork-max/danieldowork-web` |
| `yo20ywork-max/millennium-social-os-stage00-ci` | `yo20ywork-max/crossgen-lab` |
| `yo20ywork-max/discord-2-0-app` | `yo20ywork-max/community-os` |
| `yo20ywork-max/tqcpb-official-site` | `yo20ywork-max/tqcpb-web` |
| `millenniume/millennium-cms-line-ai-v4` | `millenniume/millennium-web` |
| `millenniume/millennium-social-os` | `millenniume/crossgen` |
| `yo20ywork-max/gpt-codex-web-bridge` | `yo20ywork-max/chatgpt-codex-bridge` |
| `yo20ywork-max/millennium-cms-line-ai-v4` | `yo20ywork-max/millennium-web-legacy` |
| `yo20ywork-max/shared-context` | `yo20ywork-max/dev-context` |
